...
from .transformer_dis_group import TransformerDisGroupReg, TransformerDisGroup
...
class PretrainAux(TransformerDisGroup):
    @classmethod
    def get_args(cls):
        return {
            **super().get_args(),
    ...
    def forward(self, batch, **kwargs):
        #print('1. Model forward() overloaded @ PretrainAux! ')
        reinforce = kwargs.get('reinforce', False)
        hypo, logit, target, reg_loss, stats, batch = self._forward(batch, **kwargs)
        if self.training:
            if self.net.transformer.weight_freezed \
...
class PretrainAuxContext(PretrainAux):
    def _process_reg_loss(self, o, context, features, group_mask, G):
        print('calling _process_reg_loss()')
...
# TODO: delete Context in future version
