...
class TopDownAttnLayer(nn.Module):
    def __init__(self, projected_size, attenType='dot'):
        super(TopDownAttnLayer, self).__init__()
        
        self.projected_size = projected_size
        self.attenType = attenType
        
        if attenType=='bilinear':
            self.W_qK = nn.Linear(projected_size, projected_size)
    ...
    def forward(self, K, q, V):
        ...
        q [batch_size, hidden_dim] // query q, global embeddings
        V [batch_size, num_frames, hidden_dim] // value V, local embeddings
        ...
        if self.attenType=='additive':
            ...
            q = q.unsqueeze(1).expand(batch_size, num_frames, hidden_dim)
            x = F.tanh(F.dropout(q))
            # project to one dim
            x = F.dropout(self.W_x(x))
        elif self.attenType=='new_additive':
            q = q.unsqueeze(1).expand(batch_size, num_frames, hidden_dim)
            x = F.tanh(F.dropout(K) + F.dropout(q))
            # project to one dim
            x = F.dropout(self.W_x(x))
        ...
        a = F.softmax(x, dim=1) # [batch_size, num_frames] : attention score a
        a = a.expand(batch_size, num_frames, hidden_dim) # [batch_size, num_frames, hidden_dim] : attention score a
        res = V * a # [batch_size, num_frames, hidden_dim]
