...
class TransformerDis(HybridDis):
    ...
    def merge_context(self, features, cls_embd, sep_embd):
        ###print('6. merging context')
        features = OrderedDict(sorted(features.items()))  # canonical ordering
        output = torch.cat((cls_embd, *chain(*[(feature, sep_embd) for feature in features.values()])), dim=1)
        return output
    ...
    def run_transformer(self, B, hypo, features, keyword=None, infer=False):
        ###print('5. run_transformer() ')
        ...
        #print("features['swin_base']", features['swin_base'].shape)
        #print("features['swin_char_spec']", features['swin_char_spec'].shape)
