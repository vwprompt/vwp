...
def load_vwp_text(path, sample):
    ...
    for i, row in enumerate(data):
        row = row[0]  # weird structure of VWP... : 1 element list, always
...
@ex.capture
def load_vwp_features(path, mode, image_ids, feature_names, feature_name_map, cross_modal_names):
    path = list(path.glob('*'))
    ...
    for p in path:
        ...
        if name in feature_names:
            if name in cross_modal_names:
                ...
                features[name], feature_skip_count = load_vwp_feature(name, p / mode, image_ids)
...
@ex.capture
def load_vwp_feature(name, path, image_ids, repeat_vwp_image=1, cut_feature_temporal_dim={}):
    images = {}
    skip_count = 0
    ...
    for idx in tqdm(image_ids):
        p = path / f"{idx}.npy"
        if p.is_file():
            images[idx] = load_single_feat(p, repeat_vwp_image, cut_temporal_dim=cut_temporal_dim)
        else:
            p = path / f"{idx}.pkl"
            if p.is_file():
                images[idx] = load_single_feat(p, repeat_vwp_image, numpy=False, cut_temporal_dim=cut_temporal_dim)
            else:
                skip_count += 1
    return images, skip_count


def load_single_feat(p, repeat_vwp_image=1, numpy=True, cut_temporal_dim=None):
    if numpy:
        feat = np.load(p)
    else:
        with open(p, 'rb') as f:
...
@ex.capture
def load_vilbert_feature(path, image_ids, repeat_vwp_image=1):
    images = {}
    skip_count = 0
    for idx in tqdm(image_ids):
        ...
        if len(ps) > 0:
            for p in ps:
                if p.is_file():
                    ...
                    story[story_name] = load_single_feat(p, repeat_vwp_image)
        if len(story) > 0:
            images[idx] = story
        else:
            skip_count += 1
...
@ex.capture
def get_cross_modal_feature(path, data, mode, feature_names, repeat_vwp_image=1):
    skip_count = 0
...
def load_vwp(path):
    mode = path.name.split('_')[0]  # train, test, val
    
    with open(path, 'rb') as f:
        data = pickle.load(f)
        
#     # DEBUG
#     first_keys = list(data.keys())[0]
#     print ('first_keys', first_keys)
#     print ('first_keys, len', len(data[first_keys]))
#     print ('first_keys, keys', data[first_keys][1].keys())
    
#     print ('first_keys["target"]', data[first_keys][1]['target'])
#     print ('first_keys["image_id"]', data[first_keys][1]['image_id'])
#     print ('first_keys["album_id"]', data[first_keys][1]['album_id'])
#     print ('first_keys["story_id"]', data[first_keys][1]['story_id'])
#     print ('first_keys["resnet"]', data[first_keys][1]['resnet'].shape)
#     print ('first_keys type', type(data[first_keys][1]['resnet']))
#     print ('first_keys["vid"]', data[first_keys][1]['vid'])
