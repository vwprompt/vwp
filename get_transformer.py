...
def get_transformer(model_name):
    ...
    res = {
        ...
        'gpt2_npt': partial(get_gpt2, pretrained=False),
...
def get_gpt2(dropout, change_transformer_dropout, use_vist, use_actynetcap,
             transformer_size='small', pretrained=True, use_person_token=False):
    model_name = 'gpt2'
    ...
    if not (use_vist or use_actynetcap):
        ...
        person_tokens = {}
        if use_person_token:
            male_tokens = {f'male{i}_token': f'[male{i}]' for i in range(10)}
            female_tokens = {f'female{i}_token': f'[female{i}]' for i in range(10)}
            person_tokens = dict(female_tokens, **male_tokens)
            print('person_tokens', person_tokens)
    else:
        person_tokens = {}
