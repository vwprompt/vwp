...
def get_dirname_from_args(_config):
    dirname = ''
#     for key in _config['log_keys']:
    for key in sorted(_config['log_keys'], reverse=True):
        if key != 'log_keys':
            key_tokens = key.split('_')
            key_short_str = ''.join([k[0] for k in key_tokens]).upper()
            dirname += '_'
            dirname += key_short_str
            dirname += '_'
            val = _config[key]
    ...
    dirname = dirname[1:]
    dirname = dirname[:200]  # avoid file name too long error
