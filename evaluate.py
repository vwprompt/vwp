...
def evaluate_sample(model, loss_fn, tokenizer, dataloaders, logger,
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, total=subset):
            ...
            if added_stats is not None:
                if 'reg_loss' in added_stats.keys():
                    added_stats.pop('reg_loss')
                stats = {**stats, **added_stats}
            n_step += 1
...
#                     print('vid before', vid)
#                     print('batch.vid', batch.vid)
#                     print('batch.targets.shape', batch.targets.shape)
#                     print('hypos.shape', hypos.shape)
    ...
    if len(texts.keys()) > 0:
        ...
        #texts = {k: [v[0], v[1]] for k, v in texts.items()}
        ...
        if albums is not None and len(albums) > 0:
            ...
            hypos = {k: v[0] for k, v in texts.items()}  # remove target
            hypos = refactor_text(hypos, albums, images)
            score_texts = merge_vist_output(hypos, targets)
            
            album_score_texts = merge_vist_output(hypos, targets, album=True)
    ...
    return epoch_stats, sampler_input, hypos, score_texts, batch_stats
